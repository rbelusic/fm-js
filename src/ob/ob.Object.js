 ...
 * as observer by invoking the @link ob.Object#addListener method on the subject instance - the object instance
 * whose events it wants to be notified of.
 * <br/>
 * This is an observer pattern implementation using the described FM event arhitecture.
 ...
 *
 *   subject.addListener(observer,"onEventName");<br/>
 ...
 * (ŠTA JE S GETERIMA, TEOREcKI?)<br/>
 * 
 * Attributes form the data in the data model. They are modeled as key-value pairs and stored as a Javascript object instance in the <code>options</code> Javascript property of the <code>ob.object</code> instance.
 * <br/>
 * Properties are the metadata of the data model, and they describe the synchronicity of the actual data in the data model with the actual data in some data source.
 * They are modeled as key-value pairs and stored as a Javascript object instance in the <code>prop</code> Javascript property of the <code>ob.object</code> instance.
 * <br/>
 * The default <code>ob.object</code> properties are:
 * <ul>
 ...
 * <li><code>fetched</code> - set to <code>true</code> upon instantiation, VAS DA PM NEMA OĐE, TO JE ZA VANJSKI AKTIVNI DATA SORS?</li>
 * </ul>
 * Whenever an attribute gets mutated via the @link ob.Object#setAttr method, its <code>dirty</code> property gets set to <code>true</code>, the <code>timestamp</code> property gets updated and an <code>onChange</code> event gets fired.<br/>
 * Whenever a property gets mutated via the @link ob.Object#setProperty method, an <code>onChange</code> event gets fired.<br/>
 * The <code>dirty</code> property can be updated via the @link ob.Object#setChanged method. It can be updated with or without firing an <code>onChange</code> event.
 * <br/>
 * @class FM.Object
 ...
 * @param {object} [flds] Allowed attributes, ____OPISATI_UKRATKO_SITUACIJU NA HRVATSKOM_DA_MOGU_SLOŽIT_ELEMENTARNI_PRIMJER_U_KODU____TO_JE_ONO_SA_TRUFOLS_U_KONSTRUKTORU_AL_NE_MOGU_REPRODUCIRAT
 * 
 */
